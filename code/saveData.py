...
phenotype					= data.getvalue("phenotype", "PHENOTYPE_NULL")
...
with open(fileName, 'a') as csvFile:
	...
	csvWriter.writerow(['subjectkey', 'src_subject_id', 'site', 'interview_date', 'interview_age', 'sex', 'phenotype', 'handedness', 'task_version', 'key_press', 'start_time', 'end_time', 'browser', 'index', 'first_half_probabilities', 'second_half_probabilities', 'deck_color', 'deck_position', 'deck_probabilities', 'deck_contingencies', 'response_color', 'response_position', 'response_probability', 'reward_type', 'trial_type', 'rt', 'reward_tally', 'exclusion_reason'])

	for row in range(numRows):

		csvWriter.writerow([subjectkey, autoWorkerID, site, interview_date, interview_age, sex, phenotype, handedness, task_version, keys[row], startDate, endDate, userAgentString, trialNums[row],
